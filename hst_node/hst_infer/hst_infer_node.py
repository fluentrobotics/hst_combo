...
class HST_infer_node(Node):
    def __init__(self,
        ...
        # Subscriber #######################################
        ### human skeleton msg 
        logger.info("INITINTINTITNTINTI")
    ...
    def _skeleton_callback(self, msg: MultiHumanSkeleton):
        logger.info("CALLBACK IS ACTUALLY HAPPENING")
        ...
        logger.info(f"A {A} T {T} K {K} D {D}")

        current_human_id_set = self.skeleton_databuffer.get_current_multihumanID_set()
        logger.info(f"CURRENT HUMAN ID SET: {current_human_id_set}")
        if len(current_human_id_set) == 0:
            ...
            if EVALUATION_NODE:
                ...
                try:
                    logger.info(f"HUMAN FRAME {HUMAN_FRAME}")
                    logger.info(f"BASE FRAME {STRETCH_BASE_FRAME}")
        ...
        if EVALUATION_NODE:
            logger.info("EVALUATION NODE TIME")
            ...
            with open(pickle_file_path.as_posix(), write_mod) as pickle_hd:
                ...
                logger.info(f"Dump pickle at step {self.counter}")
                self.counter += 1
